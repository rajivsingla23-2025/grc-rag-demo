...
with left_col:
    ...
    extra_instruction = st.text_area("Optional additional instruction to LLM (e.g., tone, verbosity)", height=50, value="Answer succinctly for an auditor.")
...
"""
    ...
    if "last_hits" in st.session_state:
        ...
        frameworks = sorted({
            fw for h in hits for fw in (h["meta"].get("frameworks") or [])
        })
...
#    # 🔍 DEBUG — verify frameworks are present
#    st.write(
#        "DEBUG frameworks seen:",
#        {fw for h in raw_control_hits for fw in h["meta"].get("frameworks", [])}
#    )
#    st.write("DEBUG: raw_control_hits =", len(raw_control_hits))

    requested_frameworks = detect_frameworks_from_query(user_query)
    
    control_hits = []
    for h in raw_control_hits:
        raw_fw = h["meta"].get("frameworks", [])

        if isinstance(raw_fw, str):
            frameworks = [raw_fw.upper()]
        elif isinstance(raw_fw, list):
            frameworks = [f.upper() for f in raw_fw]
        else:
            frameworks = []

        # CASE 1: framework explicitly mentioned
        if requested_frameworks:
            if any(
                rf in fw
                for rf in requested_frameworks
                for fw in frameworks
            ):
                control_hits.append(h)

        # CASE 2: no framework specified → allow all controls
        else:
            control_hits.append(h)
...
#    st.write("DEBUG: control_hits after filter =", len(control_hits))
    ...
    for h in policy_hits + control_hits:
        key = (h["meta"]["filename"], h["meta"]["chunk_id"])
        if key not in seen:
            merged.append(h)
            seen.add(key)
    ...
    with provenance_container:
        ...
        for h in merged:
            m = h["meta"]
            st.write(
                f"- {m.get('filename')} | chunk {m.get('chunk_id')} | "
                f"type={m.get('type')} | score={h['score']:.4f}"
            )
